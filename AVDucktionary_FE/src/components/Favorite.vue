...
const favoriteWords = ref([]) // ✅ Danh sách từ yêu thích (lưu full object)
const searchTerm = ref('') // ✅ Tìm kiếm trong danh sách
const selectedWord = ref(null) // ✅ Từ được chọn để xem chi tiết
const isLoading = ref(false) // ✅ Trạng thái loading
const { user } = useUser() // ✅ Lấy user đang đăng nhập
const userId = user.value.id // ✅ Lấy ID của user hiện tại
const notes = ref([])
const newNote = ref('')

// ✅ Tải danh sách từ yêu thích
const loadFavorites = async () => {
  if (!userId) return
  isLoading.value = true
  // Lấy full object để hiển thị thông tin từ trong table
  favoriteWords.value = (await getFavorites(userId)).map((fav) => fav.word)
  isLoading.value = false
}

// ✅ Thêm từ vào danh sách yêu thích (giống Dictionary)
...
const deleteAllFavorites = () => {
  Swal.fire({
    title: 'Xác nhận xoá',
    text: 'Bạn có chắc chắn muốn xoá tất cả từ yêu thích?',
    icon: 'question',
    ...
    Swal.fire({
      title: 'Chắc chắn xoá',
      text: 'Không thể khôi phục các từ đã xoá, vẫn tiếp tục?',
      icon: 'warning',
      showCancelButton: true,
      confirmButtonText: 'Xoá',
      cancelButtonText: 'Hủy',
      ...
      if (result.isConfirmed) {
        ...
        favoriteWords.value = []
        showToast('success', 'Đã xoá tất cả từ yêu thích')
      }
    })
  })
}
...
const saveNote = async () => {
  try {
    ...
    // Sử dụng hàm giống Dictionary: kiểm tra bằng .some cho object
...
<template>
  <div>
    <h2 class="text-center">Danh sách từ yêu thích</h2>
    ...
    <div class="text-end mt-3">
      <button class="btn btn-danger" @click="deleteAllFavorites">Xóa tất cả từ yêu thích</button>
    </div>
        ...
        <tbody>
          <tr v-for="word in filteredWords" :key="word.id">
            ...
            <td>
              ...
              <button @click="deleteFavorite(word.id)" class="btn btn-danger btn-sm">
                ❌ Bỏ thích
              </button>
            </td>
          </tr>
      ...
      <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <!-- Kiểm tra sự tồn tại của từ trong danh sách bằng .some -->
            <button
            ...
            <button v-else @click="deleteFavorite(selectedWord?.id)" class="btn btn-danger">
              ❌ Bỏ thích
            </button>
